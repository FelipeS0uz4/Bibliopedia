...
def SalvarComentarios(comentario):  
    ...
    data = date.today()
    data_corrigida = (data.day,data.month,data.year)
    nota = comentario['nota']
    with conexao() as conectar:
        cursor = conectar.cursor() 
        cursor.execute(
            'INSERT INTO dados.cometarios (idLivros,idUsuario,comentario,dataComentario,nota) VALUES (%s,%s, %s,%s,%s)',
            (idLivro, idUsuario,comentarios,data_corrigida,nota))
        conectar.commit()
