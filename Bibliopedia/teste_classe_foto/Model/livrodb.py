...
def AdicionarLivroNoBanco(dados):
    with conexao() as conectar:
        cursor = conectar.cursor()
        cursor.execute(
            """
            INSERT INTO dados.biblioteca (Autor, Descricao, Titulo, Genero, LinkCompra, Lingua, IdAutor, Preco) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """,
            (
                dados["Autor"],dados["Descricao"],dados["Titulo"],
                dados["Genero"],dados["LinkCompra"],dados["Lingua"],
                dados["IdAutor"],dados["Preco"],
            )
        )
        conectar.commit()
