...
def salvarUsuariodb(usuario):
    ...
    if verficacao:
        ...
        with conexao() as conectar:
            cursor = conectar.cursor() 
            cursor.execute(
                'INSERT INTO dados.usuarios (NomeUsuario, EmailUsuario, SenhaUsuario,ImagemUsuario,StatusUsuario,TypeClient) VALUES (%s,%s, %s,%s,%s,%s)',
                (username, email, senhaHashed,Imagem,Status,TypeClient,))
            conectar.commit()
            cursor.execute(
...
def buscarUsuariodbPorEmail(email):
    with conexao() as conectar:
        cursor = conectar.cursor(dictionary=True)
        cursor.execute("SELECT * FROM dados.usuarios WHERE EmailUsuario = %s", (email,))
...
def atualizasenha(id,senhanova):
    senhaHashed = generate_password_hash(senhanova, method='pbkdf2:sha256')
    with conexao() as conectar:
        cursor = conectar.cursor()
        cursor.execute("UPDATE dados.usuarios SET SenhaUsuario = %s  WHERE idUsuario = %s", 
                       (senhaHashed,id))
        conectar.commit()
        cursor.close()


def emailesqueci(email_usuario):
    dados = buscarUsuariodbPorEmail(email_usuario)
    id = dados["idUsuario"]
        ...
        <table>
            ...
            <tr>
                <td colspan="2" style="text-align: center;">
                    ...
                    <p>
                        <a href="http://localhost:5173/recover/{id}" class="button">
                            Recuperar Conta
                        </a>
                    </p>
                    <p>Se você não solicitou este email, ignore-o.</p>
