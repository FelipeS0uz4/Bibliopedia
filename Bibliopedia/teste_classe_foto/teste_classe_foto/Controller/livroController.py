...
class Livro():
    ...
    def TodosLivros(idUsuario, googleToken):
        ...
        if googleToken:
            try:
                ...
                if response.status_code == 200:
                    ...
                    print("Erro ao acessar favoritos do Google Books:", response.json())
            except Exception as e:
                print("Erro ao buscar favoritos:", e)
        else:
            print("Token do Google não encontrado na sessão.")

        todos_livros = livros_local + livros_google
...
def informacoes_autor(nome_autor):
    try:
        ...
        birthDate        = item.get('birth_date', '--/--/----')
        TopPreferido     = item.get('top_subjects', [])
