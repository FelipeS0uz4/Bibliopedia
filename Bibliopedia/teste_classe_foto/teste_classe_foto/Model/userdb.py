...
def atualizarUmUsuariodb(id, usuario):
    ...
    with conexao() as conectar:
        cursor = conectar.cursor()
        cursor.execute("UPDATE dados.usuarios SET NomeUsuario  = %s, EmailUsuario = %s, ImagemUsuario = %s  WHERE idUsuario = %s", 
                       (username, email,Imagem,id,))
        conectar.commit()
        cursor.close()
        ...
        <table>
            ...
            <tr>
                <td colspan="2" style="text-align: center;">
                    ...
                    <p>
                        <a href="http://localhost:5173/Reset/{id}" class="button">
                            Recuperar Conta
                        </a>
                    </p>
                    <p>Se você não solicitou este email, ignore-o.</p>
